...
class Filler:
    ...
    def update_comment_biased(self,comment:str,placeholders:dict):
        ...
        if 'player_modifier' in comment and positive_player!='-1':
            comment = comment.replace("{player_modifier"+str(positive_player)+"}", random.choice(self.__modifier["player_modifier"]["good"]))

        if 'player_modifier' in comment and positive_player!='-1':
            comment = comment.replace("{team_modifier"+str(positive_player)+"}", random.choice(self.__modifier["team_modifier"]["good"]))
        ...
        for placeh in placeholders:
            ...
            if placeh[:6] == 'player':
                comment = comment.replace("{"+placeh+"}", random.choice(self.__modifier["player_modifier"]["bad"]))
            if placeh[:4] == 'team':
                comment = comment.replace("{"+placeh+"}", random.choice(self.__modifier["team_modifier"]["bad"]))
        return comment
